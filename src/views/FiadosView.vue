...
<template>
  <div class="page fade-up">
    <header class="page-head">
      ...
      <button class="btn btn-primary mini-btn" @click="mostrarNuevo = true">+ Registrar Cliente</button>
    </header>
    ...
    <div class="resumen card">
      <div class="resumen-contenido">
        ...
        <span class="r-sub">
          {{ clientes.length }} {{ clientes.length === 1 ? 'cuenta activa con balance en mora' : 'cuentas activas con balance en mora' }}
        </span>
      </div>
    </div>
    ...
    <div v-else-if="!clientes.length" class="vacio card">
      <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="vacio-icon"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
      ...
      <li v-for="c in clientes" :key="c.id" class="cliente card" @click="abrirCliente(c)">
        ...
        <div class="c-info">
          ...
          <span v-if="c.telefono" class="c-tel">
            <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display:inline; margin-right:3px; vertical-align:middle;"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
            {{ c.telefono }}
          </span>
        </div>
        ...
        <div class="hist-wrapper">
          <ul v-if="historial.length" class="hist">
            <li v-for="m in historial" :key="m.id" class="hist-item">
              ...
              <span class="h-monto" :class="m.tipo">{{ m.tipo === 'abono' ? '-' : '+' }}{{ fmt(m.monto) }}</span>
              <span class="h-fecha">{{ new Date(m.fecha).toLocaleDateString('es-CR') }}</span>
            </li>
          </ul>
          <p v-else class="dim center fs-xs">Sin registros de transacciones previas en este ciclo.</p>
        </div>

        <button class="btn btn-ghost btn-block" @click="seleccionado = null">Cerrar Expediente</button>
      </div>
    </div>
    ...
    <div v-if="mostrarNuevo" class="modal-bg" @click.self="mostrarNuevo = false">
      <div class="modal card">
        ...
        <div class="field">
          ...
          <input id="f-nuevo-nombre" v-model="nuevoNombre" placeholder="Ej. Juan Carlos Alvarado" required />
        </div>
...
.resumen {
  background: linear-gradient(135deg, rgba(245, 158, 11, 0.08) 0%, rgba(17, 20, 34, 0) 100%);
  border: 1px solid rgba(245, 158, 11, 0.2);
  padding: 1.5rem;
  margin-bottom: 1.5rem;
}
.resumen-contenido { display: flex; flex-direction: column; gap: 0.2rem; }
.r-label { color: #94a3b8; font-size: 0.85rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.04em; }
.r-val { font-size: clamp(1.8rem, 4vw, 2.3rem); font-weight: 800; color: #f59e0b; line-height: 1.1; letter-spacing: -0.03em; }
.r-sub { color: #64748b; font-size: 0.8rem; }

.state-msg { text-align: center; padding: 2.5rem 0; color: #64748b; font-size: 0.9rem; }

/* Estado Vacío */
.vacio { padding: 3rem 1.5rem; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 0.75rem; }
.vacio-icon { color: var(--accent, #a3e635); opacity: 0.8; }
.v-title { font-weight: 700; font-size: 1.1rem; color: #ffffff; margin: 0; }
...
.cliente:active { transform: translateY(0); }
...
.avatar.grande { width: 56px; height: 56px; font-size: 1.4rem; }

.c-info { flex: 1; display: flex; flex-direction: column; gap: 0.15rem; min-width: 0; }
.c-nombre { font-weight: 600; color: #ffffff; text-overflow: ellipsis; overflow: hidden; white-space: nowrap; }
.c-tel { color: #64748b; font-size: 0.8rem; }
.c-saldo { font-weight: 700; color: #f59e0b; font-size: 1.05rem; letter-spacing: -0.01em; }

/* Control de Amortización Integrada */
.det-head { display: flex; align-items: center; gap: 1.2rem; margin-bottom: 1.5rem; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 1.2rem; }
.det-head h3 { margin: 0 0 0.2rem 0; font-size: 1.2rem; color: #ffffff; font-weight: 700; }
.saldo-grande { font-size: 1.6rem; font-weight: 800; color: #f59e0b; letter-spacing: -0.02em; }

.abono-container { margin-bottom: 1.5rem; display: flex; flex-direction: column; gap: 0.4rem; }
.field-label { color: #cbd5e1; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
.abono-row { display: flex; gap: 0.5rem; }
.abono-row .inp {
  flex: 1;
...
.abono-row .inp:focus { outline: none; border-color: #f59e0b; }

/* Histórico de Movimientos ERP */
.hist-title { font-size: 0.8rem; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 0.6rem; font-weight: 600; }
.hist-wrapper { max-height: 180px; overflow-y: auto; margin-bottom: 1.5rem; padding-right: 0.25rem; border-radius: 6px; background: rgba(0,0,0,0.1); border: 1px solid rgba(255,255,255,0.03); }
.hist { list-style: none; padding: 0 0.6rem; margin: 0; display: flex; flex-direction: column; }
.hist-item { display: grid; grid-template-columns: 80px 1fr auto; gap: 0.8rem; padding: 0.65rem 0; border-bottom: 1px solid rgba(255,255,255,0.04); align-items: center; }
.hist-item:last-child { border-bottom: none; }

.h-tipo { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; padding: 0.15rem 0.5rem; border-radius: 4px; text-align: center; width: fit-content; }
.h-tipo.consumo { background: rgba(239, 68, 68, 0.1); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.15); }
.h-tipo.abono { background: rgba(34, 197, 94, 0.1); color: #4ade80; border: 1px solid rgba(34, 197, 94, 0.15); }

.h-monto { font-weight: 600; font-size: 0.9rem; text-align: right; }
.h-monto.consumo { color: #f1f5f9; }
.h-monto.abono { color: #4ade80; }
.h-fecha { color: #64748b; font-size: 0.78rem; justify-self: end; }

.btn-block { width: 100%; height: 40px; justify-content: center; font-weight: 600; }
...
.modal { width: 100%; max-width: 440px; padding: 1.6rem; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.4); }
...
.hist-wrapper::-webkit-scrollbar-track { background: transparent; }
.hist-wrapper::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 2px; }
</style>